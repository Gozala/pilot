...
function Command(descriptor, name) {
    if (typeof descriptor === "function") descriptor = { exec: descriptor };
...
exports['plugin:startup'] = function onPluginStartup(event) {
    ...
    if ((commands = event.plugin.commands)) {
        Object.keys(commands).forEach(function (name) {
            env._emit('command:install', {
                env: env,
                command: Command(commands[name], name),
                commands: env.commands.enabled
            });
        });
    }
};
...
exports['command:install'] = function onCommandInstall(event) {
    ...
    event.env.commands.enabled[command.name] = command;
};

exports['command:uninstall'] = function onCommandUninstall(event) {

    delete event.env.commands.enabled[event.command.name];
};
