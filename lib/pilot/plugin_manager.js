...
function PluginCatalog(plugins) {
    return Object.create(PluginCatalog.prototype, {
        plugins: { value: plugins }
    });
};
PluginCatalog.prototype = Object.create(EventEmitter, {
    constructor: { value: PluginCatalog },
    signal: { value: function signal(names, action) {
        ...
        Object.keys(names).forEach(function(name) {
            ...
            try {
                ...
                this._emit("error", action, plugin, exception)
            }
        }, this);
    } },
    register: { value: function register(plugins) {
        plugins.forEach(function(plugin) {
            ...
            if (!(name in this.plugins)) this.plugins[name] = plugin;
        }, this);
    }, enumerable: true },
    ...
    plug: { value: function plug(data, plugins) {
        ...
        this.signal(plugins, "plug", data);
        this.signal(plugins, "startup", data);
    }, enumerable: true },
    unplug: { value: function unplug(data, plugins) {
        this.signal(plugins || this.plugins, "unplug", data);
        this.signal(plugins || this.plugins, "shutdown", data);
    }, enumerable: true }
});
