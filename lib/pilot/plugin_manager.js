...
function initialize(env) {
    ...
    env.on(NS + ':startup', onStartup);
    env.on(NS + ':shutdown', onShutdown);
    env.on(NS + ':install', onInstall);
    env.on(NS + ':uninstall', onUninstall);
 ...
 */
function install(env, descriptor, reason) {
 ...
 *      `true` if install was successful and `false` if install failed.
 */
function uninstall (env, name, reason) {
 ...
 */
function startup(env, name, reason) {
 ...
 *      Reason for plugin startup.
 */
function shutdown(env, name, reason) {
