...
exports.Plugin.prototype = {
    ...
    /**
     * Signals to other components of the editor that this plugin changed
     * status.
     * @param {String} topic
     *      Event type emitted on the environment to signal listeners.
     * @param {Environment} env
     *      Environment using which event is emitted.
     */
    signal: function(topic, env) {
        env.emit(topic, { plugin: this });
        env.emit(topic + ':' + this.name, { plugin: this });
    },
    ...
    install: function(data, reason) {
        ...
        require([this.name], function(pluginModule) {
            ...
            this.signal("plugin:install", data.env);
        }.bind(this));
        return pr;
    },
    ...
    register: function(data, reason) {
        ...
        if (this.status != this.INSTALLED) {
            pr.resolve(this);
            this.signal("plugin:register", data.env);
            return pr;
        }
        require([this.name], function(pluginModule) {
    ...
    startup: function(data, reason) {
        ...
        if (this.status != this.REGISTERED) {
            ...
            // Notifying listeners that plugin has been started up.
            this.signal("plugin:install", data.env);
            return pr;
        }
        require([this.name], function(pluginModule) {
    ...
    shutdown: function(data, reason) {
        ...
        this.signal("plugin:shutdown", data.env);
    }
};
